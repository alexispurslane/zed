...
mod tests {
    ...
    /// Integration test: multi-stroke split keybindings (like ctrl-x 3 in
    /// the Emacs keymap or cmd-k right in the default keymap) must work
    /// correctly. This is a regression test for a bug where `pending_has_binding`
    /// was always `false` when no complete binding matched the prefix key,
    /// preventing the 1-second flush timeout from being set. Without the
    /// timeout, any focus change between keystrokes would clear the pending
    /// input, causing the second key to be dispatched as standalone text
    /// input (e.g. typing a literal "3" instead of completing
    /// "ctrl-x 3 → pane::SplitRight").
    #[gpui::test]
    async fn test_split_via_multi_stroke_keybinding(cx: &mut TestAppContext) {
        init_test(cx);

        // Bind a multi-stroke key sequence to SplitRight, simulating
        // the Emacs-style ctrl-x 3 binding.
        cx.update(|cx| {
            cx.bind_keys([
                gpui::KeyBinding::new(
                    "ctrl-x 3",
                    SplitRight::default(),
                    None, // Available in all contexts for testing
                ),
            ]);
        });
        ...
        add_labeled_item(&pane_before, "A", false, cx);

        // Focus the pane so the dispatch path includes the Pane's
        // action handlers for SplitRight.
        pane_before.update_in(cx, |pane, window, cx| {
            window.focus(&pane.focus_handle(cx), cx);
        });

        // Before the fix, pending_has_binding was false, so no 1-second
        // timeout was set. Any focus change (including the which-key
        // modal opening) would clear the pending ctrl-x, and the "3"
        // key would be dispatched as text input to the editor.
        //
        // After the fix, pending_has_binding is true when there are
        // pending bindings, so the timeout is set and the two
        // keystrokes are correctly combined.
        cx.simulate_keystrokes("ctrl-x 3");
        cx.executor().run_until_parked();

        // The split should have created a new pane
        let pane_count = workspace.read_with(cx, |workspace, _| workspace.panes().len());
        assert_eq!(
            pane_count, 2,
            "ctrl-x 3 should have split the pane, creating 2 panes, but found {pane_count}"
        );
    }
    ...
    /// Regression test: when a modifier key (like shift) is pressed and
    /// released between the keystrokes of a multi-key binding sequence
    /// (e.g. ctrl-x 3), the synthetic modifier keystroke generated by
    /// `ModifiersChangedEvent` handling must NOT flush the pending input.
    /// This is critical for keyboard layouts that require shift to type
    /// digits, where the sequence is: ctrl-down, x-down, x-up, ctrl-up,
    /// shift-down, 3-down, 3-up, shift-up.
    #[gpui::test]
    async fn test_multi_stroke_binding_with_modifier_between(cx: &mut TestAppContext) {
        init_test(cx);
        ...
        // Simulate the real sequence of events when a user with a
        // keyboard layout that requires shift to type digits presses
        // ctrl-x then 3:
        // 1. ctrl press
        cx.simulate_modifiers_change(gpui::Modifiers::control());
        cx.executor().run_until_parked();
        // 2. x press (while ctrl held)
        cx.simulate_keystrokes("ctrl-x");
        cx.executor().run_until_parked();
        // 3. ctrl release
        cx.simulate_modifiers_change(gpui::Modifiers::none());
        cx.executor().run_until_parked();
        // 4. shift press (to type the digit 3 on this keyboard layout)
        cx.simulate_modifiers_change(gpui::Modifiers::shift());
        cx.executor().run_until_parked();
        // 5. shift release — this used to generate a synthetic "shift"
        //    keystroke that destroyed the pending ctrl-x input
        cx.simulate_modifiers_change(gpui::Modifiers::none());
        cx.executor().run_until_parked();
        // 6. 3 press
        ...
        assert_eq!(
            pane_count, 2,
            "ctrl-x 3 should have split the pane even with a modifier key pressed between the two keystrokes, but found {pane_count}"
        );
    }
