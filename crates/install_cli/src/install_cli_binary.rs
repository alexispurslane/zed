...
async fn install_script(cx: &AsyncApp) -> Result<PathBuf> {
    ...
    let link_path = Path::new("/usr/local/bin/xed");
...
pub fn install_cli_binary(window: &mut Window, cx: &mut Context<Workspace>) {
    ...
    cx.spawn_in(window, async move |workspace, cx| {
        ...
        workspace.update_in(cx, |workspace, _, cx| {
            struct InstalledXedCli;

            workspace.show_toast(
                Toast::new(
                    NotificationId::unique::<InstalledXedCli>(),
                    format!(
                        "Installed `xed` to {}. You can launch {} from your terminal.",
    ...
    .detach_and_prompt_err("Error installing xed cli", window, cx, |_, _, _| None);
}
