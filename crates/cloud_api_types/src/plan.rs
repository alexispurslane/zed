...
mod tests {
    ...
    fn test_plan_deserialize_snake_case() {
        let plan = serde_json::from_value::<Plan>(json!("xenomorphic_free")).unwrap();
        assert_eq!(plan, Plan::XenomorphicFree);

        let plan = serde_json::from_value::<Plan>(json!("xenomorphic_pro")).unwrap();
        assert_eq!(plan, Plan::XenomorphicPro);

        let plan = serde_json::from_value::<Plan>(json!("xenomorphic_pro_trial")).unwrap();
        assert_eq!(plan, Plan::XenomorphicProTrial);

        let plan = serde_json::from_value::<Plan>(json!("xenomorphic_student")).unwrap();
        assert_eq!(plan, Plan::XenomorphicStudent);
    }
}
