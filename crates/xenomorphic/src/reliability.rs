...
pub fn init(_client: Arc<Client>, cx: &mut gpui::App) {
    if cfg!(debug_assertions) {
        log::info!("Debug assertions enabled, skipping hang monitoring");
    } else {
        monitor_hangs(cx);
    }
    // Crash/minidump upload removed (was telemetry-dependent).
}

fn monitor_hangs(cx: &gpui::App) {
...
fn save_hang_trace(
    ...
    let thread_timings = thread_timings
        ...
        .map(|mut timings| {
            ...
            gpui::SerializedThreadTaskTimings::convert(*STARTUP_TIME.get().unwrap(), timings)
        })
        .collect::<Vec<_>>();
