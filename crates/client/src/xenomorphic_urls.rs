//! Contains helper functions for constructing URLs to various Xenomorphic-related pages.
//!
...
//! the configured server in development).
...
/// Returns the URL to the account page on the configured server.
pub fn account_url(cx: &App) -> String {
    format!("{server_url}/account", server_url = server_url(cx))
}

/// Returns the URL to the start trial page on the configured server.
...
/// Returns the URL to the upgrade page on the configured server.
pub fn upgrade_to_xenomorphic_pro_url(cx: &App) -> String {
    format!("{server_url}/account/upgrade", server_url = server_url(cx))
}

/// Returns the URL to Xenomorphic's terms of service.
pub fn terms_of_service(cx: &App) -> String {
    format!("{server_url}/terms-of-service", server_url = server_url(cx))
}

/// Returns the URL to Xenomorphic AI's privacy and security docs.
...
/// Returns the URL to Xenomorphic's edit prediction documentation.
